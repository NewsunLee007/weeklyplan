...
router.post('/:planId/approve', authMiddleware, requireRole(...REVIEW_ROLES), (req, res) => {
  ...
  const plan = queryOne(`SELECT * FROM biz_week_plan WHERE id = ? AND is_deleted = 0`, [planId]);
  ...
  // 记录审核日志
  run(
    `INSERT INTO biz_review_record (plan_id, reviewer_id, step, result, comment, create_time) VALUES (?, ?, ?, 'APPROVED', ?, ?)`,
  ...
  // 发布后推送企微
  if (flow.to === 'PUBLISHED') {
    const updatedPlan = queryOne(`SELECT * FROM biz_week_plan WHERE id=?`, [planId]);
...
router.post('/:planId/reject', authMiddleware, requireRole(...REVIEW_ROLES), (req, res) => {
  ...
  const n = now();
  run(
    `INSERT INTO biz_review_record (plan_id, reviewer_id, step, result, comment, create_time) VALUES (?, ?, ?, 'REJECTED', ?, ?)`,
    [planId, userId, step, comment, n]
  );
  run(`UPDATE biz_week_plan SET status='REJECTED', update_time=? WHERE id=?`, [n, planId]);
