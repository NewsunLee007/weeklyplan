...
router.put('/:id', authMiddleware, requireRole('ADMIN'), (req, res) => {
  ...
  const user = queryOne(`SELECT id FROM sys_user WHERE id = ? AND is_deleted = 0`, [id]);
...
router.delete('/:id', authMiddleware, requireRole('ADMIN'), (req, res) => {
  ...
  run(`UPDATE sys_user SET is_deleted=1, update_time=? WHERE id=?`, [now(), id]);
...
router.put('/:id/reset-password', authMiddleware, requireRole('ADMIN'), (req, res) => {
  ...
  run(`UPDATE sys_user SET password=?, update_time=? WHERE id=?`, [hashedPwd, now(), id]);
