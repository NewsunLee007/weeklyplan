...
router.post('/', authMiddleware, (req, res) => {
  ...
  const existing = queryOne(
    `SELECT id FROM biz_feedback WHERE plan_item_id = ? AND feedback_user_id = ?`,
    [plan_item_id, userId]
  );

  if (existing) {
    run(`UPDATE biz_feedback SET status=?, content=?, update_time=? WHERE id=?`,
      [status, content || '', n, existing.id]);
  } else {
    run(
      `INSERT INTO biz_feedback (plan_item_id, plan_id, feedback_user_id, status, content, create_time, update_time) VALUES (?, ?, ?, ?, ?, ?, ?)`,
...
router.get('/plan/:planId', authMiddleware, (req, res) => {
